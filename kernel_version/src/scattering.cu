...
void scatter(rmm::device_uvector<int>& buffer,rmm::device_uvector<int>& predicate)
{
    ...
    kernel_scattering<int,-27><<<nb_block,512,0,buffer.stream()>>>(raft::device_span<int>(buffer.data(),buffer.size()),raft::device_span<int>(predicate.data(),predicate.size()-1));
} 
